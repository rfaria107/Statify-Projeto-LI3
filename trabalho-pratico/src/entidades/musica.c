...
int parse_musica_and_add_him(RowReader* reader, GestorMusicas* gestorMusic) {
    ...
    if (is_empty_value(id_str)) return 1;
    int id = atoi(id_str);
    ...
    if (is_empty_value(artist_ids_str)) return 1;
    ...
    if (is_empty_value(year_str)) return 1;
...
char **parse_liked_musics(RowReader* reader) {
    ...
    if (liked_musics_id == NULL || strlen(liked_musics_id) <= 2) return NULL;
